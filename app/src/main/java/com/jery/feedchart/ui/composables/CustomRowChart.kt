...
fun CustomRowChart(
    ...
    barOverlayText: (String, Double) -> String = { label, value -> "$label: $value" },
    barOverlayStyle: TextStyle = MaterialTheme.typography.labelSmall,
                ...
                Canvas(modifier = Modifier
                    ...
                    data.forEachIndexed { dataIndex, bars ->
                        bars.values.forEachIndexed { barIndex, bar ->
                            ...
                            bar.label?.let {
                                val label = barOverlayText(it, bar.value)
                                val labelWidth = textMeasurer.measure(label, style = barOverlayStyle).size.width
                                drawText(
                                    textMeasurer.measure(label, style = barOverlayStyle),
//                                    color = Color.Gray,
                                    topLeft = Offset(
                                        x = barX + 8 + (width.absoluteValue.toFloat() - labelWidth) / 30,
                                        y = barY + everyBarHeight / 4,
                                    )
                                )
                            }
                        }
